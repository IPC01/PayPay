  ...
  const loadWallets = async () => {
    try {
      ...
      notify({
        ...
        message: 'Não foi possível carregar as suas carteiras para C2B.'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedWalletCode) {
      notify({
        ...
        message: 'Escolha uma carteira para efetuar C2B.'
      });
      return;
    }
    ...
    try {
      setSubmitting(true);
      await authRequest('/api/payments/c2b', {
        method: 'POST',
        body: {
      ...
      notify({
        type: 'success',
        title: 'C2B realizado',
        message: 'Transação C2B simulada com sucesso e saldo atualizado.'
      });
      setAmount('');
      setPhone('');
      setReference('');
      ...
      notify({
        type: 'error',
        title: 'Erro no C2B',
        message: err.message || 'Não foi possível realizar o C2B.'
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Transações C2B</h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
            Selecione uma carteira ativa e complete o formulário para simular uma transação C2B.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
        <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Formulário C2B</h3>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            >
              {submitting ? 'Processando...' : 'Realizar C2B'}
            </button>
          </form>
        </div>
...
export default TransactionsC2B;
