        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <select
              ...
              className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white sm:w-80"
            >
            ...
            <div className="mt-6 space-y-4 text-sm text-slate-700 dark:text-slate-300">
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.user?.name || selectedRequest.userId}</p>
                  <p className="text-slate-500 dark:text-slate-400">{selectedRequest.user?.email || '-'}</p>
                </div>
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Status</h3>
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.status?.toUpperCase()}</p>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.Admin?.name || selectedRequest.adminId ? `ID ${selectedRequest.adminId}` : '-'}</p>
                  <p className="text-slate-500 dark:text-slate-400">{selectedRequest.Admin?.email || '-'}</p>
                </div>
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Processado em</h3>
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.processedAt ? new Date(selectedRequest.processedAt).toLocaleString('pt-PT') : '-'}</p>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.wallet?.walletName || selectedRequest.wallet?.walletCode || '-'}</p>
                </div>
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Montante</h3>
                  <p className="mt-2 text-base font-medium text-slate-900 dark:text-white">{selectedRequest.amount}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="mt-2 rounded-3xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                  {selectedRequest.adminMessage || 'Sem observações'}
                </p>
              </div>
            </div>
          </div>
