...
function AdminUsers() {
  ...
  const UserDetailsModal = ({ open, loading, detail, onClose }) => {
    if (!open) return null;
        ...
        <div className="mb-5 flex items-start justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-700">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">
              {detail?.user?.name || 'Utilizador'}
            </h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{detail?.user?.email}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-900 dark:text-white">{detail.user?.Role?.name || 'N/A'}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Carteiras</p>
                <p className="mt-2 text-sm font-semibold text-slate-900 dark:text-white">{detail.wallets.length}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Transações</p>
                <p className="mt-2 text-sm font-semibold text-slate-900 dark:text-white">{detail.transactions.length}</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
              ...
              {detail.wallets.length === 0 ? (
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  {detail.wallets.slice(0, 6).map((wallet) => (
